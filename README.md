...
> ⚠️ Selalu pakai WHERE saat UPDATE, kalau tidak semua baris akan berubah!

### DELETE — Menghapus Data
DELETE FROM nama_tbl WHERE id = 1;
> ⚠️ Selalu pakai WHERE saat DELETE, kalau tidak semua data akan terhapus!
...
-- ORDER BY
SELECT * FROM produk ORDER BY harga ASC;   -- murah ke mahal
SELECT * FROM produk ORDER BY harga DESC;  -- mahal ke murah
SELECT * FROM produk ORDER BY nama ASC, harga DESC; -- 2 kolom

-- LIMIT & OFFSET
SELECT * FROM produk LIMIT 5;           -- ambil 5 data pertama
SELECT * FROM produk LIMIT 5 OFFSET 10; -- skip 10, ambil 5 berikutnya
-- berguna untuk pagination

-- COLUMN ALIAS
SELECT nama AS nama_produk, harga AS harga_jual FROM produk;
SELECT COUNT(*) AS total_produk FROM produk;

-- WHERE dengan pencarian string (LIKE)
SELECT * FROM produk WHERE nama LIKE 'a%';   -- diawali huruf a
SELECT * FROM produk WHERE nama LIKE '%a';   -- diakhiri huruf a
SELECT * FROM produk WHERE nama LIKE '%tas%'; -- mengandung kata "tas"
